 ...
 protected:
  ...
  void Fill(const bool unique_pixels, DataParameter_DB backend) {
    ...
    for (int i = 0; i < 5; ++i) {
      ...
      Datum* data_datum = new Datum();
      data_datum->set_label(i);
      data_datum->set_channels(2);
      data_datum->set_height(3);
      data_datum->set_width(4);
      std::string* data = data_datum->mutable_data();
      for (int j = 0; j < 24; ++j) {
        int datum = unique_pixels ? j : i;
        data->push_back(static_cast<uint8_t>(datum));
      }
      ml_datum.set_allocated_data(data_datum);

      Datum* label_datum = new Datum();
      label_datum->set_channels(5);
      label_datum->set_height(1);
      label_datum->set_width(1);
      std::string* label = label_datum->mutable_data();
      for (int j = 0; j < 5; ++j) {
        label->push_back(static_cast<uint8_t>(i + j));
      }
      ml_datum.set_allocated_label(label_datum);

      stringstream ss;
      ss << i;
      string out;
      CHECK(ml_datum.SerializeToString(&out));
      txn->Put(ss.str(), out);
    }
    txn->Commit();
    db->Close();
  ...
  void TestReshape(DataParameter_DB backend) {
    ...
    for (int i = 0; i < num_inputs; ++i) {
      ...
      data_datum->set_label(i);
      data_datum->set_channels(2);
      data_datum->set_height(i % 2 + 1);
      data_datum->set_width(i % 4 + 1);
      std::string* data = data_datum->mutable_data();
      const int data_size = data_datum->channels() *
                            data_datum->height() *
                            data_datum->width();
  ...
  void TestReadCropTrainSequenceSeeded() {
    ...
    {
      MultilabelDataLayer<Dtype> layer1(param);
    ...
    Caffe::set_random_seed(seed_);
    MultilabelDataLayer<Dtype> layer2(param);
  ...
  virtual ~MultilabelDataLayerTest() { delete blob_top_data_; delete blob_top_label_; }
...
#ifdef USE_LEVELDB
TYPED_TEST(MultilabelDataLayerTest, TestReadLevelDB) {
  const bool unique_pixels = false;  // all pixels the same; images different
  this->Fill(unique_pixels, DataParameter_DB_LEVELDB);
  this->TestRead();
}

/*TYPED_TEST(MultilabelDataLayerTest, TestSkipLevelDB) {
  this->Fill(false, DataParameter_DB_LEVELDB);
  this->TestSkip();
}*/

TYPED_TEST(MultilabelDataLayerTest, TestReshapeLevelDB) {
  this->TestReshape(DataParameter_DB_LEVELDB);
}

TYPED_TEST(MultilabelDataLayerTest, TestReadCropTrainLevelDB) {
...
TYPED_TEST(MultilabelDataLayerTest, TestReadCropTrainSequenceSeededLevelDB) {
  const bool unique_pixels = true;  // all images the same; pixels different
  this->Fill(unique_pixels, DataParameter_DB_LEVELDB);
...
TYPED_TEST(MultilabelDataLayerTest, TestReadCropTrainSequenceUnseededLevelDB) {
  const bool unique_pixels = true;  // all images the same; pixels different
  this->Fill(unique_pixels, DataParameter_DB_LEVELDB);
  this->TestReadCropTrainSequenceUnseeded();
}

TYPED_TEST(MultilabelDataLayerTest, TestReadCropTestLevelDB) {
  const bool unique_pixels = true;  // all images the same; pixels different
  this->Fill(unique_pixels, DataParameter_DB_LEVELDB);
...
#ifdef USE_LMDB
TYPED_TEST(MultilabelDataLayerTest, TestReadLMDB) {
  const bool unique_pixels = false;  // all pixels the same; images different
  this->Fill(unique_pixels, DataParameter_DB_LMDB);
  this->TestRead();
}

/*TYPED_TEST(MultilabelDataLayerTest, TestSkipLMDB) {
  this->Fill(false, DataParameter_DB_LMDB);
  this->TestSkip();
}*/

TYPED_TEST(MultilabelDataLayerTest, TestReshapeLMDB) {
  this->TestReshape(DataParameter_DB_LMDB);
}

TYPED_TEST(MultilabelDataLayerTest, TestReadCropTrainLMDB) {
  const bool unique_pixels = true;  // all images the same; pixels different
  this->Fill(unique_pixels, DataParameter_DB_LMDB);
  this->TestReadCrop(TRAIN);
}
...
// Test that the sequence of random crops is consistent when using
// Caffe::set_random_seed.
TYPED_TEST(MultilabelDataLayerTest, TestReadCropTrainSequenceSeededLMDB) {
  const bool unique_pixels = true;  // all images the same; pixels different
  this->Fill(unique_pixels, DataParameter_DB_LMDB);
  this->TestReadCropTrainSequenceSeeded();
}
...
// Test that the sequence of random crops differs across iterations when
// Caffe::set_random_seed isn't called (and seeds from srand are ignored).
TYPED_TEST(MultilabelDataLayerTest, TestReadCropTrainSequenceUnseededLMDB) {
  const bool unique_pixels = true;  // all images the same; pixels different
  this->Fill(unique_pixels, DataParameter_DB_LMDB);
  this->TestReadCropTrainSequenceUnseeded();
}

TYPED_TEST(MultilabelDataLayerTest, TestReadCropTestLMDB) {
  const bool unique_pixels = true;  // all images the same; pixels different
  this->Fill(unique_pixels, DataParameter_DB_LMDB);
  this->TestReadCrop(TEST);
}
