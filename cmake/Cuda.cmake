...
################################################################################################
# A function for automatic detection of all GPUs installed in the PatRec machines
# Usage:
#   caffe_detect_installed_gpus(out_variable)
function(caffe_detect_patrec_gpus out_variable)
    ...
    file(STRINGS ${PROJECT_SOURCE_DIR}/patrec_gpu_machines.txt __patrec_gpu_machines)
    message(STATUS "Found following GPU machines: ${__patrec_gpu_machines}")
    set(__nvcc_out "")
    set(__nvcc_res 0)
    foreach(__machine ${__patrec_gpu_machines})
      execute_process(COMMAND ssh "${__machine}" "${CUDA_NVCC_EXECUTABLE}" "--run" "-Wno-deprecated-gpu-targets" "${__cufile}"
                      RESULT_VARIABLE __nvcc_res_machine
                      OUTPUT_VARIABLE __nvcc_out_machine)
      message(STATUS "GPU architectures on ${__machine}: ${__nvcc_out_machine}")
      if(__nvcc_res_machine EQUAL 0)
        set(__nvcc_out "${__nvcc_out} ${__nvcc_out_machine}")
      else()
        set(_nvcc_res 1)
      endif()
    endforeach(__machine)
...
function(caffe_select_nvcc_arch_flags out_variable)
  # List of arch names
  set(__archs_names "Fermi" "Kepler" "Maxwell" "Pascal" "All" "Manual" "PatRec")
  set(__archs_name_default "All")
  if(NOT CMAKE_CROSSCOMPILING)
    list(APPEND __archs_names "Auto")
    set(__archs_name_default "PatRec")    
  endif()
  ...
  elseif(${CUDA_ARCH_NAME} STREQUAL "PatRec")
    caffe_detect_patrec_gpus(__cuda_arch_bin)
